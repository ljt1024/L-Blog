...
传统多页应用（MPA）的页面切换：
─────────────────────────────────
错误 白屏闪烁：点击链接 → 浏览器卸载旧文档 → 网络请求 → 加载新文档 → 白屏
错误 无过渡动画：两个页面之间没有任何过渡效果
错误 用户感知慢：即使页面内容已经加载，也要等到整个页面渲染完才有反馈
错误 无法保留状态：切换页面后，前一个页面的滚动位置/表单内容完全丢失
```
...
| 页面级过渡 | 正确 原生支持 | 注意 需手动处理 | 注意 需手动处理 | 错误 不支持 |
| DOM 元素映射 | 正确 view-transition-name | 注意 手动 FLIP | 正确 AnimatePresence | 错误 不支持 |
| MPA 支持 | 正确 跨文档 | 错误 | 错误 | 错误 |
| 移动端性能 | 正确 GPU 合成 | 正确 GPU | 正确 GPU | 正确 GPU |
| 学习曲线 | ⭐⭐（简单） | ⭐⭐⭐⭐（陡峭） | ⭐⭐⭐（中等） | ⭐⭐（简单） |
| 复杂交互 | 注意 有限 | 正确 无限 | 正确 无限 | 注意 有限 |
...
─────────────────────────────────
正确 页面级切换（MPA / Next.js 页面路由）
正确 简单的元素位移/缩放过渡
正确 需要零依赖的场景
正确 移动端优先的项目（原生 GPU 加速）

GSAP / Framer Motion 更适合：
─────────────────────────────────
正确 复杂的时间线动画
正确 拖拽交互（Draggable / Motion）
正确 复杂的状态机动画
正确 ScrollTrigger / 视差效果
正确 SVG 路径动画
```
...
─────────────────────────────────
Chrome/Edge：      正确 完全支持（2024）
Safari：           正确 完全支持（17+）
Firefox：          注意 部分支持（需要实验性标志）
Mobile Chrome：    正确 完全支持
Mobile Safari：   正确 完全支持（17+）
...
使用场景：
─────────────────────────────────
正确 Next.js / Nuxt.js 页面路由
正确 MPA 站点（传统 HTML 网站）
正确 移动端 Web App（性能敏感）
正确 需要保留旧页面视觉状态的场景
正确 无障碍访问（降低认知负担）
