...
|------|-------------|--------------|-------------------|----------------|------------|
| 作用域 | 正确 文件级 | 正确 类名级 | 正确 组件级 | 正确 文件级 | 正确 层叠级 |
| TypeScript | 注意 一般 | 正确 | 正确 | 正确 完美 | 正确 |
| 主题系统 | 错误 | 正确 设计 token | 正确 运行时 | 注意 编译时 | 错误 |
| 零运行时 | 正确 | 正确（JIT） | 错误 | 正确 | 正确 |
| 学习曲线 | ⭐ 低 | ⭐⭐⭐（工具类） | ⭐⭐（模板语法） | ⭐⭐（TS + CSS） | ⭐（原生语法） |
| IDE 支持 | 注意 一般 | 正确 IntelliSense | 正确 | 正确 完美 | 正确 |
| Tree-shaking | 注意 | 正确（JIT） | 错误 | 正确 | 正确 |
...
用 CSS Modules：
  正确 已有项目（渐进迁移，无破坏性改动）
  正确 想要零运行时 + 接近原生 CSS 体验
  正确 不想要学工具类语法
  正确 Vue/React/普通 HTML 通用
...
用 Vanilla Extract：
  正确 想要 TypeScript 完美支持
  正确 想要零运行时 + CSS-in-JS 的 DX
  正确 新项目（接受编译步骤）
  正确 需要设计系统（themes）

用 CSS-in-JS（Styled Components）：
  正确 需要复杂的运行时主题切换
  正确 动态样式逻辑复杂（基于 props 变化）
  正确 小型项目（~13KB 运行时可接受）

用 CSS Layers：
  正确 想要原生解决方案（无构建工具依赖）
  正确 需要优先级控制（reset > base > components）
  正确 和其他方案结合使用
```
