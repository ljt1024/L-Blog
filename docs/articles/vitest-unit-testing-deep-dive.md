...
| 特性 | Jest | Vitest |
|------|------|--------|
| ESM 原生支持 | 错误 需配置 | 正确 开箱即用 |
| TypeScript 支持 | 注意 需 Babel | 正确 esbuild 直接转换 |
| 热更新速度 | 错误 慢 | 正确 毫秒级 |
| Vite 插件兼容 | 错误 不支持 | 正确 直接使用 |
| Mock 方式 | 侵入式 | 正确 函数式 + Vite 插件 |
| 快照测试 | 正确 | 正确 |
| 并行测试 | 正确 | 正确（基于 Workers）|
| 浏览器测试 | 错误 | 正确（通过 WebdriverIO）|
| 覆盖率 | 正确 | 正确（c8）|
```
...
// 正确 正确：beforeEach 中重置
beforeEach(() => {
  arr = [1, 2, 3]
})

// 错误 错误 3：共享状态导致测试顺序依赖
let sharedState = 0
